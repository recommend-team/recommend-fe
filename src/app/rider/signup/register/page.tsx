...
import { useRegisterRider } from "@/hooks";
...
function RegisterFormInner() {
  ...
  const tierParam = searchParams.get("tier") as RiderType | null;
  const tier: RiderType =
    tierParam === "COMPANY" || tierParam === "INDIVIDUAL"
      ? tierParam
      : "INDIVIDUAL";
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      const result = await register.mutateAsync({
        ...
        guarantorName: values.guarantorName.trim() || undefined,
        guarantorPhone: values.guarantorPhone.trim() || undefined,
      });
      router.push(
        `/rider/signup/verify?email=${encodeURIComponent(result.email)}`
      );
    } catch (err) {
      setSubmitError(
        ...
        <div className="max-w-xl mx-auto flex flex-col gap-6">
          <div className="flex flex-col items-center gap-1 text-center">
            ...
            <Text variant="neighborhoods-list" color="grey">
              {tier === "COMPANY" ? "Fleet / company" : "Solo rider"}
            </Text>
          </div>
          ...
          >
            ...
            <Text variant="neighborhoods-list" color="grey">
              Optional now — speeds up KYC later:
            </Text>

            <FormField
              label="BVN"
              error={errors.bvn?.message}
              hint="11 digits. Your Bank Verification Number is never shared publicly."
            >
              <input
                inputMode="numeric"
                maxLength={11}
